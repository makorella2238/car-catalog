...
const CarItem: FC<ICarSinge> = ({car}) => {
    return (
        <div className={ ` ${ s.center } p-8 border border-slate-300 text-center relative rounded-lg shadow-md shadow-slate-300/50` }>
            <h2 className={ ` ${ styles.gradientText } leading-5 py-0.5 sm:leading-7 text-lg absolute top-3 sm:text-xl md:text-2xl text-center border border-b-2 border-blue-800 px-1 mx-2 rounded-l` }>{ car.title }</h2>
            ...
            <div className='absolute bottom-3'>
                <i className='sm:text-xl font-semibold'>{ car.price }$</i>

                <Link className='ml-8' href={ `/car/${ car._id }` }>
                    <button className={`${ styles.baseButton} p-1 sm:p-2 `}>Read more</button>
                </Link>
            </div>
